...
@section('title', 'Tambah User Baru — IndoTech Admin')

@section('content')
<form method="POST" action="{{ route('admin.users.store') }}" class="space-y-6 max-w-7xl mx-auto">
    @csrf
    ...
    <div>
        ...
        <div class="text-[13px] text-slate-500 mb-1 flex items-center gap-1.5 font-medium">
            ...
            <span class="text-slate-900 font-semibold">Tambah User Baru</span>
        </div>

        <h1 class="text-[26px] font-bold text-slate-900 tracking-tight">Tambah User Baru</h1>
    </div>
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-5">
                <div class="flex items-center gap-2.5 mb-2 text-slate-900">
                    ...
                    <h2 class="text-[17px] font-bold text-slate-900">Informasi Akun User</h2>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-[13px] font-semibold text-slate-700 mb-2">Password <span class="text-red-500">*</span></label>
                        <input type="password" 
                               name="password" 
                               required
            ...
            {{-- Card 2: Organization Info (Optional) --}}
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-5">
                <div class="flex items-center gap-2.5 mb-2 text-slate-900">
                    <svg width="22" height="22" fill="none" viewBox="0 0 24 24" stroke="#0b57d0" stroke-width="2" class="shrink-0">
                    ...
                    <h2 class="text-[17px] font-bold text-slate-900">Informasi Organisasi / Instansi (Opsional)</h2>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               placeholder="Nama PIC organisasi"
                               class="w-full bg-[#f0f4f9]/80 hover:bg-[#e9eef6] focus:bg-white border border-transparent focus:border-blue-500 rounded-xl px-4 py-3 text-[14px] text-slate-800 outline-none transition font-medium">
                        @error('org_contact')
                            <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="org_address" 
                                  ...
                                  class="w-full bg-[#f0f4f9]/80 hover:bg-[#e9eef6] focus:bg-white border border-transparent focus:border-blue-500 rounded-xl px-4 py-3 text-[14px] text-slate-800 outline-none transition font-medium">{{ old('org_address') }}</textarea>
                        @error('org_address')
                            <p class="text-xs text-red-500 mt-1 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs space-y-5">
                ...
                <div>
                    ...
                    <select name="role" required class="w-full bg-[#f0f4f9]/80 hover:bg-[#e9eef6] focus:bg-white border border-transparent focus:border-blue-500 rounded-xl px-4 py-3 text-[14px] text-slate-800 outline-none transition cursor-pointer font-medium">
                        <option value="user" {{ old('role') == 'user' ? 'selected' : '' }}>User (Siswa / umum)</option>
                        <option value="school_admin" {{ old('role') == 'school_admin' ? 'selected' : '' }}>School Admin</option>
                        <option value="university_rep" {{ old('role') == 'university_rep' ? 'selected' : '' }}>University Representative</option>
                        <option value="company_hr" {{ old('role') == 'company_hr' ? 'selected' : '' }}>Company HR</option>
