...
<form action="{{ route('admin.jobs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="flex items-center justify-between gap-4">
        <a href="{{ route('admin.jobs.index') }}" class="inline-flex items-center gap-2 text-[14px] font-semibold text-slate-700 hover:text-blue-600 transition">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-2xs space-y-5">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="mis. Senior Frontend Engineer" required
                    ...
                    <div>
                        ...
                        <input type="text" name="salary_range" value="{{ old('salary_range') }}" placeholder="mis. 15.000.000 - 25.000.000"
                               class="w-full bg-slate-100/70 border border-slate-200 rounded-xl px-4 py-2.5 text-[13.5px] text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="category" class="w-full bg-slate-100/70 border border-slate-200 rounded-xl px-4 py-2.5 text-[13.5px] text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">
                            <option value="jobs" {{ old('category') === 'jobs' ? 'selected' : '' }}>Jobs (Reguler)</option>
                            <option value="internship" {{ old('category') === 'internship' ? 'selected' : '' }}>Internship (Magang)</option>
                            <option value="freelance" {{ old('category') === 'freelance' ? 'selected' : '' }}>Freelance / Kontrak</option>
                            <option value="remote" {{ old('category') === 'remote' ? 'selected' : '' }}>Remote Work</option>
                            <option value="graduate" {{ old('category') === 'graduate' ? 'selected' : '' }}>Graduate Job</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="experience" class="w-full bg-slate-100/70 border border-slate-200 rounded-xl px-4 py-2.5 text-[13.5px] text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">
                            <option value="Entry" {{ old('experience') === 'Entry' ? 'selected' : '' }}>Entry Level</option>
                            <option value="Mid" {{ old('experience', 'Mid') === 'Mid' ? 'selected' : '' }}>Mid Level</option>
                            <option value="Senior" {{ old('experience') === 'Senior' ? 'selected' : '' }}>Senior Level</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-2xs space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="6" required id="desc-field" placeholder="Tuliskan deskripsi lengkap pekerjaan..."
                              class="w-full bg-slate-100/70 border border-slate-200 rounded-xl p-4 text-[13.5px] text-slate-900 leading-relaxed focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">{{ old('description') }}</textarea>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-2xs space-y-4">
                ...
                <div>
                    ...
                    <textarea name="requirements" rows="6" placeholder="- Minimal 3 tahun pengalaman di bidang terkait...&#10;- Menguasai React, TypeScript, dan Git..."
                              class="w-full bg-slate-100/70 border border-slate-200 rounded-xl p-4 text-[13.5px] text-slate-900 leading-relaxed focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">{{ old('requirements') }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-2xs space-y-4">
                ...
                <div class="space-y-2 pt-1">
                    ...
                    <div id="image-preview-container" class="w-full h-36 rounded-xl border border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center relative mb-2 shadow-inner">
                        <img id="image-preview" src="{{ old('image') }}" alt="Preview" class="w-full h-full object-cover {{ old('image') ? '' : 'hidden' }}" onerror="handleImageError(this)">
                        <div id="image-placeholder" class="flex flex-col items-center justify-center text-slate-400 {{ old('image') ? 'hidden' : '' }}">
                ...
                <div>
                    ...
                    <input type="text" name="company" value="{{ old('company') }}" placeholder="mis. TechFlow Systems" required
                           class="w-full bg-slate-100/70 border border-slate-200 rounded-xl px-4 py-2.5 text-[13.5px] text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="location" value="{{ old('location') }}" placeholder="mis. Jakarta Selatan, DKI Jakarta" required
                ...
                <div>
                    ...
                    <select name="company_size" class="w-full bg-slate-100/70 border border-slate-200 rounded-xl px-4 py-2.5 text-[13.5px] text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">
                        ...
                        <option value="51-200 Employees" {{ old('company_size', '51-200 Employees') === '51-200 Employees' ? 'selected' : '' }}>51-200 Employees</option>
