...
@section('title', 'Edit School Information — IndoTech')

@section('content')
<form method="POST" action="{{ route('admin.schools.update', $school['id']) }}" class="space-y-6 max-w-7xl mx-auto">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <h1 class="text-[26px] font-bold text-slate-900 tracking-tight">Edit School Information</h1>
    </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="text" 
                               ...
                               required
                               placeholder="SMK Telkom Jakarta"
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="institution_type" 
                                ...
                                <option value="SMK IT" {{ old('institution_type', $school['institution_type'] ?? '') == 'SMK IT' ? 'selected' : '' }}>SMK IT</option>
                                <option value="SMK Non-IT" {{ old('institution_type', $school['institution_type'] ?? '') == 'SMK Non-IT' ? 'selected' : '' }}>SMK Non-IT</option>
                                <option value="SMA" {{ old('institution_type', $school['institution_type'] ?? '') == 'SMA' ? 'selected' : '' }}>SMA</option>
                                <option value="Universitas" {{ old('institution_type', $school['institution_type'] ?? '') == 'Universitas' ? 'selected' : '' }}>Universitas</option>
                            </select>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs">
                ...
                <div>
                    ...
                    <textarea name="description" 
                              ...
                              class="w-full bg-[#f0f4f9]/80 hover:bg-[#e9eef6] focus:bg-white border border-transparent focus:border-blue-500 rounded-xl p-4 text-[14px] text-slate-800 outline-none transition font-medium leading-relaxed resize-none">{{ old('description', $school['description'] ?? 'SMK Telkom Jakarta is a premier vocational high school focusing on Information Technology and Telecommunications. We prepare students for the modern tech workforce with a specialized curriculum in Network Engineering, Software Development, and Multimedia.') }}</textarea>
                    
                    <div class="flex justify-end mt-2">
                        <span id="charCount" class="text-[12.5px] font-medium text-slate-500">
                            {{ strlen(old('description', $school['description'] ?? '')) }} / 1000 characters
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs">
                ...
                <div class="w-full aspect-[4/3] rounded-2xl bg-sky-50 border border-slate-200/80 overflow-hidden relative mb-4">
                    {{-- Stylized Map SVG/Illustration matching screenshot --}}
                    <svg viewBox="0 0 320 220" class="w-full h-full object-cover">
                        ...
                        {{-- Labels --}}
                        <text x="80" y="45" font-size="9" fill="#0891b2" font-weight="bold">Aloha Pasir Putih</text>
                        <circle cx="160" cy="42" r="4" fill="#a855f7"/>

                        <text x="240" y="85" font-size="8" fill="#64748b" font-weight="bold">Batavia PIK</text>
                        <text x="230" y="115" font-size="8" fill="#0284c7" font-weight="bold">PIK Avenue</text>
                        <circle cx="285" cy="112" r="4" fill="#0284c7"/>

                        <text x="270" y="115" font-size="8" fill="#0284c7" font-weight="bold">Baywalk</text>

                        {{-- Province Boundaries --}}
                        <text x="35" y="175" font-size="8" fill="#94a3b8" font-weight="bold" transform="rotate(-65 35,175)">BANTEN</text>
                        <text x="75" y="180" font-size="8" fill="#94a3b8" font-weight="bold" transform="rotate(-65 75,180)">JAKARTA</text>

                        {{-- Airport icon --}}
                        <circle cx="25" cy="165" r="8" fill="#2563eb"/>
                        <path d="M25,160 L27,164 L30,165 L27,166 L25,170 L24,166 L21,165 L24,164 Z" fill="#ffffff"/>
                    </svg>
                </div>
                ...
                <div class="flex items-start gap-2 text-slate-700">
                    ...
                    <span class="text-[13.5px] font-medium leading-relaxed">
                        {{ $school['address'] ?? 'Jl. Daan Mogot, Cengkareng, Jakarta Barat, 11730' }}
                    </span>
                </div>
            </div>
